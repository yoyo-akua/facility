		...
		*/
		public function new_number($department,$specification){
			...
			*/
			...
			## Convert the last nuber into an array.
			$slash_array=explode("/",$last_number);
			
			
			## Define the array variables as number, year and letter, if it is an OPD number (which has the letter in the beginning).
			if($department=='OPD'){
				$old_letter=$slash_array[0];
				$old_number=$slash_array[1];
				$old_year=$slash_array[2];
			}
			
			## In an other case just define the variables year and number from the array.
			else{
				$old_number=$slash_array[0];
				$old_year=$slash_array[1];
			}
			
			/*
			## That part of the new patient's number, which does not equal the current year, is increased.
			## If the year changes, the increasing part of the new patient's number is reset to 1 and the new year is set as the other part of the new patient's number.
			*/
			$year=date("y",time());
			if($old_year!==$year){
				$number=1;
			...
			## Put the variables (letter,) number and year back together into one variable and return this complete variable.
			if($department=='OPD'){
				$new_number=$old_letter.'/'.$number.'/'.$year;
			}else{
				$new_number=$number.'/'.$year;
			}
